...
public class MapGenerator : MonoBehaviour {
	...
	public void initMap () 
	{
		...
		List<Tile> outerTiles = map.getVertices().Where (t => t.neighbours.Count< 6).ToList();
		...
		while(tileRemoved < tilesToRemove)
		{
			...
			//than <6.
			List<Tile> sideTiles = map.getVertices().Where (t =>t.neighbours.Count<4 ).ToList();
	...
	public void initializeVillagesOnMap(List<Player> players)
	{
		foreach ( Tile t in this.map.getVertices() )
		{
			...
			if ( t.visited == false  && t.color != players.Count )
			{
			...
			if (t.myVillage == null && t.color != players.Count)
			{
	...
	public void searchVillages(Tile toSearch, List<Tile> TilesToReturn, int color )
	{
		...
		{
			if(n.visited == false && n.color == color)
			{
				n.visited=( true );
				TilesToReturn.Add(n);
